...
\tightlist
\item
  Simulating a sample of observations (\(\mathbfit{x}\)) from
  \(N_p(\mu, \Sigma)\).
\item
...
To convert this to points on the surface of a confidence ellipse,
...
Finally, new observations can be visually compared with this ellipse by
...
\includegraphics[width=0.8\textwidth,height=\textheight]{paper_files/figure-pdf/fig-ci-1.pdf}
...
observation to have length equal to 1. A confidence ellipse is generated
by transforming the sphere relative to a specified variance-covariance
matrix (c), and new observations can be visually assessed to be inside
or outside by plotting with the ellipse (d).}
