...
2005; D. Cook et al. 2006; Lee et al. 2022). The combination of these
...
provide context. This is called a guided tour (Dianne Cook et al. 1995).
...
<<<<<<< HEAD
average Mahahlanobis distance (Mahalanobis 1936) in the projection for a
=======
average Mahalanobis distance \citep{mahalanobis} in the projection for a
>>>>>>> 669463dcb39becd2e2796cb494e2c5247e9065bb
...
This is implemented in the \texttt{tourr} (Hadley Wickham et al. 2011;
H. Wickham et al. 2024) package, where the projected ellipsoid can be
drawn for each projection. The guided tour will take arguments
specifying the data, and the null variance-covariance matrix.
...
This example is motivated by the weather data example from Filzmoser,
Hron, and Templ (2018). We illustrate using the anomaly index to compare
...
Filzmoser, Hron, and Templ (2018) for more details).
...
estimator (Peter J. Rousseeuw 1985) using the implementation in Maechler
et al. (2024). The MCD estimates for the mean vector and the
...
different combinations of variables, as found in Filzmoser, Hron, and
Templ (2018), we will further separate the outlying points into clusters
based on similarity in direction. The similarity is computed by first
...
we reproduce one of the results from Filzmoser, Hron, and Templ (2018)
in Figure~\ref{fig-shapley}. On top we can see the cluster assignments
for all points that we have identified as outliers using the
\(p\)-dimensional Mahalanobis distance.

For example we see that cluster 4 contains the years 1962 and 1996, and
for these two years the result from Filzmoser, Hron, and Templ (2018)
also indicates unusually low values for the minimum teperature, while
...
\caption{\label{fig-shapley}Reproduction of the results from Filzmoser,
Hron, and Templ (2018), showing which cells have been identified as
...
The work is related to outlier detection methods (see, Filzmoser, Hron,
and Templ 2018, for example), particularly those that use robust
statistics, as illustrated in the weather example. Methods discussed in
Peter J. Rousseeuw, Raymaekers, and Hubert (2018) and Donoho and Gasko
(1992) describe detecting outlyingness using projections of the data.
When these projections are collected and used as a large set, they
define a high-dimensional confidence region of an indeterminate shape.
There is no analytical definition. However, we could explore the data
relative to the high-dimensional shapes using the same approach
described in Section~\ref{sec-background}. You would generate points on
the surface of the irregular shape, overlay the data on this to
visualise with a tour.
...
Buja, A., and D. Asimov. 1986. {``{G}rand {T}our {M}ethods: {A}n
{O}utline.''} \emph{Computing Science and Statistics} 17: 63--67.

\bibitem[\citeproctext]{ref-BCAH05}
Buja, A., D. Cook, D. Asimov, and C. Hurley. 2005. {``{C}omputational
{M}ethods for {H}igh-{D}imensional {R}otations in {D}ata
{V}isualization.''} In \emph{Handbook of Statistics: Data Mining and
Visualization}, edited by C. R. Rao, E. J. Wegman, and J. L. Solka,
...
Cook, D., E.-K. Lee, A. Buja, and H. Wickham. 2006. {``{G}rand {T}ours,
{P}rojection {P}ursuit {G}uided {T}ours and {M}anual {C}ontrols.''} In
\emph{Handbook of {D}ata {V}isualization}, edited by C.-H. Chen, W.
Härdle, and A. Unwin. Berlin: Springer.
...
\bibitem[\citeproctext]{ref-filzmoser2018}
Filzmoser, P., K. Hron, and M Templ. 2018. \emph{Applied Compositional
Data Analysis}. Cham, Switzerland: Springer.
\url{https://doi.org/10.1007/978-3-319-96422-5}.

\bibitem[\citeproctext]{ref-FR02}
Fraley, C., and A. E. Raftery. 2002. {``Model-Based {C}lustering,
...
Gower, J. C., and D. J. Hand. 1996. \emph{Biplots}. London: Chapman;
Hall.
...
Jones, M. C., and R. Sibson. 1987. {``{W}hat Is {P}rojection {P}ursuit?
(With Discussion).''} \emph{Journal of the Royal Statistical Society,
Series A} 150: 1--36.

\bibitem[\citeproctext]{ref-lib-med-liver-norms}
Lala, V., M. Zubair, and D. A. Minter. 2023. {``Liver Function Tests.''}
...
Koller, Eduardo L. T. Conceicao, and Maria Anna di Palma. 2024.
...
Wickham, H., D. Cook, N. Spyrison, N. Laa, H. S. Zhang, and S. Lee.
2024. \emph{Tour Methods for Multivariate Data Visualisation}.
