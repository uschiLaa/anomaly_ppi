...
Euclidean distance. The cluster validation statistics computed using
\citet{fpc} suggest that \(k=4\) or \(k=5\) is reasonable, so for
simplicity we will work with \(k=4\).
...
many directions. It provides a global picture showing where the outlying
points differ from the normal distribution.
