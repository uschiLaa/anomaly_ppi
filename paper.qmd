...
Let \(\mathbfit{x}\) be a \(p\) dimensional vector. A \pD{} ellipsoid corresponding to a given
variance-covariance (\(\Sigma\)) is described by the equation
...
Let \(P\) be a \((p\times 2)\) matrix, whose columns form an orthonormal basis for the \gD{} projection space. Let \(\mathbfit{y}\) be a 2 dimensional vector, and let \(\mathbfit{\mu}_p := \mathbfit{\mu} P\) denote the projected mean.
\begin{theorem}
