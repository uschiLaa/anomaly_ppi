...
This paper describes a new projection pursuit index which is optimized by projections where a new sample is most distant from the existing normal distribution.
...
It is organised as follows. @sec-background provides more context for the methods and visualisation. @sec-anomaly-index provides the details of the new index, and @sec-implementation describes the implementation. Example use is illustrated in @sec-examples. 
...
2. Transforming each observation to have unit distance from the mean, $\frac{\mathbfit{x}^\top}{||\mathbfit{x}^\top||}$. The sample is now uniform on the surface of the hypersphere.
...
Although this is flexible, this does not make it easy to guide the tour towards the directions (projections) where the samples are most different from the normal. What would be desirable is to analytically define the confidence ellipsoid, flag observations that are outside, steer the tour to projections that reveal the extent of the difference. And also display the projected ellipsoid as a geometric shape rather than a sample of points. These are the procedures that are described in the next section.  
