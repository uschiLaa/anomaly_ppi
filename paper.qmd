...
Let $x$ be a \pD{} vector. A \pD{} ellipsoid corresponding to a given variance-covariance ($\Sigma$) is defined by 
...
We call points in the projection that are on the curve $\mathbfit{y}$, so that $\mathbfit{y} = \mathbfit{x}P$ for $\mathbfit{x}$ on the \pD{} curve. Then $\mathbfit{y} - \mathbfit{\mu}_p = (\mathbfit{x} - \mathbfit{\mu})P = \mathbfit{s} P^T \Sigma P$, where $\mathbfit{\mu}_p = \mathbfit{\mu} P$ is the projected mean. We then substitute $(\mathbfit{y} - \mathbfit{\mu}_p) (P^T \Sigma P)^{-1}$ for $\mathbfit{s}$ in Equation~(\ref{eq:c2}). From this we can compute the analog equation for the projection as
...
p <- plot(weather_SCD_rescaled, type = "cell", n_digits = 0, continuous_rowname = 10, rotate_x = FALSE) +
  geom_text(data = cluster_df,
            ...
            inherit.aes = F) +
  scale_color_manual(values = c("#80146E", "#7262B2", "#31A0C2", "#5ECBBB", "#B9E5B9")) +
  theme(legend.position = "none",
        aspect.ratio = 0.5,
...
![](images/weather3.png){#fig-weather3 width=600 align="center"}
